...
class ProteinMatrix:
    ...
    def _init_matrix(self):
        self.protein_matrix = pd.DataFrame(columns=self.list_of_all_proteins_in_matrix, index=self.list_of_all_proteins_in_matrix)
        self.protein_matrix.fillna(0,inplace=True)
        
        # take each pair of proteins, find their indexes, and then populate the matrix with the proteins
        for n in range(len(self.protein_data_df)): 
            ...
            self.protein_matrix.iloc[index1, index2] = self.protein_data_df.iloc[n, 2]
